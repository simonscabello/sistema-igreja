<x-app-layout>
    <x-page-card title="Editar Visitante">
        @if($errors->any())
        ...
        <form action="{{ route('visitors.update', $visitor) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-text-input label="Nome" name="name" :value="old('name', $visitor->name)" placeholder="Digite o nome" required="true" />
                <x-text-input label="Celular" name="mobile" :value="old('mobile', $visitor->mobile)" placeholder="Digite o celular" />
                <x-select 
                    label="Faixa Etária" 
                    name="age_group" 
                    :options="['crianca_adolescente' => 'Criança / Adolescente', 'jovem' => 'Jovem', 'adulto' => 'Adulto', 'idoso' => 'Idoso']" 
                    :selected="old('age_group', $visitor->age_group)" 
                />
                <x-select 
                    label="Gênero" 
                    name="gender" 
                ...
                />
                <x-input-date label="Data da Visita" name="visit_date" :value="old('visit_date', $visitor->visit_date ? $visitor->visit_date->format('d/m/Y') : '')" />
                <x-checkbox label="Deseja ser contactado?" name="wants_contact" :checked="old('wants_contact', $visitor->wants_contact)" />
            </div>
