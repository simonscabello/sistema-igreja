<x-app-layout>
    <x-page-card title="Relatório Anual Compilado">
        ...
        @if(isset($report) && $report['yearly_totals']['entradas'] > 0 || $report['yearly_totals']['saidas'] > 0)
            ...
            <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <h3 class="text-lg font-semibold text-blue-800 dark:text-blue-300">
                    Relatório Anual Compilado de {{ $report['ano'] }}
                </h3>
            </div>
            ...
            @if(count($report['category_totals']) > 0)
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($report['category_totals'] as $categoria => $totals)
                            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                                <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ $categoria }}</h4>
                                
                                <div class="space-y-3">
                                    @if($totals['entradas'] > 0)
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-green-700 dark:text-green-300">Entradas:</span>
                                    ...
                                    @if($totals['saidas'] > 0)
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-red-700 dark:text-red-300">Saídas:</span>
                                    ...
                                    @php
                                        $saldoCategoria = $totals['entradas'] - $totals['saidas'];
                                    @endphp
                                    
                                    <div class="pt-3 border-t border-gray-200 dark:border-gray-700">
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm font-semibold text-gray-900 dark:text-white">Saldo:</span>
                                            <span class="font-bold {{ $saldoCategoria >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
            ...
            <div class="space-y-6">
                @foreach($report['monthly_data'] as $month => $data)
                    @if($data['has_transactions'] && (count($data['entradas']) > 0 || count($data['saidas']) > 0))
                        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <!-- Entradas -->
                                <div>
                                    <h5 class="text-md font-medium text-green-600 dark:text-green-400 mb-3">Entradas por Categoria</h5>
                                    @if(count($data['entradas']) > 0)
                                        <div class="space-y-2">
                                            @foreach($data['entradas'] as $categoria => $valor)
                                    ...
                                    @else
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Nenhuma entrada registrada</p>
                                    @endif
                                </div>
                                
                                <!-- Saídas -->
                                <div>
                                    <h5 class="text-md font-medium text-red-600 dark:text-red-400 mb-3">Saídas por Categoria</h5>
                                    @if(count($data['saidas']) > 0)
                                        <div class="space-y-2">
                                            @foreach($data['saidas'] as $categoria => $valor)
                                                <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ $categoria }}</span>
                                    ...
                                    @else
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Nenhuma saída registrada</p>
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-400">Escolha o ano para visualizar o relatório anual compilado.</p>
            </div>
        @endif
    </x-page-card>
</x-app-layout> 