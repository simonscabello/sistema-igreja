<x-app-layout>
    <x-page-card title="{{ isset($clonedSet) ? 'Clonar Repertório' : 'Novo Repertório' }}">
        <form method="POST" action="{{ route('worship-sets.store') }}" class="space-y-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <x-text-input id="singer" name="singer" type="text" class="mt-1 block w-full" :value="old('singer', $clonedSet?->singer ?? '')" required autofocus label="Cantor" required="true" />
                    <x-input-error :messages="$errors->get('singer')" class="mt-2" />
                </div>

                <div>
                    <x-text-input id="preacher" name="preacher" type="text" class="mt-1 block w-full" :value="old('preacher', $clonedSet?->preacher ?? '')" required label="Ministro" required="true" />
                    <x-input-error :messages="$errors->get('preacher')" class="mt-2" />
                </div>

                <div>
                    <x-text-input id="date" name="date" type="date" class="mt-1 block w-full" :value="old('date', $clonedSet?->date?->format('Y-m-d') ?? '')" required label="Data" required="true" />
                    <x-input-error :messages="$errors->get('date')" class="mt-2" />
                </div>

                <div>
                    <x-select id="period" name="period" class="mt-1 block w-full" required label="Período" required="true" :options="['manha' => 'Manhã', 'noite' => 'Noite']" :selected="old('period', $clonedSet?->period ?? '')">
                    </x-select>
                ...
                @if(isset($clonedSet) && $clonedSet->songs && $clonedSet->songs->count() > 0)
                    @foreach($clonedSet->songs as $index => $song)
                        <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-700 rounded">
                            ...
                            <input type="text" name="song_keys[{{ $song->id }}]" placeholder="Tom" class="w-16 text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded" value="{{ $song->pivot->key_used ?? '' }}">
                        </div>
                    @endforeach
                @endif
            </div>
            ...
            <div class="flex items-center justify-end gap-4">
                ...
                <x-primary-button type="submit">
                    {{ isset($clonedSet) ? 'Criar Repertório' : 'Cadastrar' }}
                </x-primary-button>
            </div>
        </form>
    </x-page-card>
