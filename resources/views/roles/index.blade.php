<x-app-layout>
    <x-page-card title="Roles" actions="{{ route('roles.create') }}">
        @if(session('success'))
        ...
        <div class="flex justify-between items-center mb-4">
            <div class="flex-1">
                <form action="{{ route('roles.index') }}" method="GET" class="flex gap-2">
                    <x-text-input name="search" placeholder="Buscar roles..." value="{{ request('search') }}" />
            ...
            <table class="min-w-full divide-y divide-neutral-medium dark:divide-gray-700">
                <thead class="bg-neutral-light dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-neutral-dark dark:text-gray-300 uppercase tracking-wider">Permissões</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-neutral-dark dark:text-gray-300 uppercase tracking-wider">Usuários</th>
                    ...
                    @forelse($roles as $role)
                        <tr class="hover:bg-neutral-light dark:hover:bg-gray-700 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-base font-semibold text-neutral-dark dark:text-gray-300">{{ $role->display_name ?? $role->name }}</div>
                                <div class="text-xs text-neutral-medium dark:text-gray-400">{{ $role->name }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <x-link-button href="{{ route('roles.show', $role) }}" class="mr-2">
                                    Ver
                                </x-link-button>
                                <x-link-button href="{{ route('roles.edit', $role) }}" class="mr-2">
                                    Editar
                                </x-link-button>
                                <form action="{{ route('roles.destroy', $role) }}" method="POST" class="inline"
                                      onsubmit="return confirm('Tem certeza que deseja excluir este role?')">
                                    @csrf
                                    @method('DELETE')
