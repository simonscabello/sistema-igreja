        ...
        <form method="POST" action="{{ route('songs.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="key" name="key" type="text" class="mt-1 block w-full" :value="old('key')" maxlength="10" placeholder="Ex: C, Dm, F#m" />
                    <x-input-error :messages="$errors->get('key')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="youtube_link" value="Link do YouTube" />
                    <x-text-input id="youtube_link" name="youtube_link" type="url" class="mt-1 block w-full" :value="old('youtube_link')" placeholder="https://youtube.com/watch?v=..." />
                    <x-input-error :messages="$errors->get('youtube_link')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="spotify_link" value="Link do Spotify" />
                    <x-text-input id="spotify_link" name="spotify_link" type="url" class="mt-1 block w-full" :value="old('spotify_link')" placeholder="https://open.spotify.com/track/..." />
                    <x-input-error :messages="$errors->get('spotify_link')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="lyrics_link" value="Link da Letra" />
                    <x-text-input id="lyrics_link" name="lyrics_link" type="url" class="mt-1 block w-full" :value="old('lyrics_link')" placeholder="https://..." />
                    <x-input-error :messages="$errors->get('lyrics_link')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="chords_link" value="Link da Cifra" />
                    <x-text-input id="chords_link" name="chords_link" type="url" class="mt-1 block w-full" :value="old('chords_link')" placeholder="https://..." />
            ...
            <div class="flex items-center gap-4">
                <x-primary-button>Cadastrar Música</x-primary-button>
